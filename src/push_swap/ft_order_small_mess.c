...
/*   Updated: 2021/11/10 09:45:35 by sde-alva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_order_small_mess(t_stack *stk_a, t_stack *stk_b)
{
	...
	ft_get_idxs(stk_a, &idxs_a, 'a', stk_a->top);
	ft_get_idxs(stk_b, &idxs_b, 'b', stk_a->top);
	...
	while (!ft_check_sort_stack_a(stk_a))
	{
		...
		if (idxs_a.max != stk_a->top && idxs_b.min != stk_b->top \
			...
			ft_swap_one(stk_b, 'b');
		else if (!ft_check_partial_sort(stk_a, 'a') && !ft_check_partial_sort(stk_b, 'b') && idxs_a.first_wrong != stk_a->top && idxs_a.first_wrong >= stk_a->top / 2 && stk_b->top > 1 \
			&& idxs_b.first_wrong >= stk_b->top / 2 && idxs_a.first_wrong != stk_a->top)
		{
		...
		else if (!ft_check_partial_sort(stk_a, 'a') && !ft_check_partial_sort(stk_b, 'b') && idxs_a.first_wrong < stk_a->top / 2 && idxs_a.first_wrong != stk_a->top && stk_b->top > 1 \
			&& idxs_b.first_wrong < stk_b->top / 2 && idxs_a.first_wrong != stk_a->top)
		{
		...
		else if (!ft_check_partial_sort(stk_a, 'a') && idxs_a.first_wrong >= stk_a->top / 2 && idxs_a.first_wrong != stk_a->top)
		{
			ft_rotate(stk_a, 'a');
			idxs_a.first_wrong = (idxs_a.first_wrong + 1) % (stk_a->top + 1);
		}
		else if (!ft_check_partial_sort(stk_b, 'b') && idxs_b.first_wrong >= stk_b->top / 2 && idxs_a.first_wrong != stk_a->top)
		{
			ft_rotate(stk_b, 'b');
			idxs_b.first_wrong = (idxs_b.first_wrong + 1) % (stk_b->top + 1);
		}
		...
		else if (!ft_check_partial_sort(stk_b, 'b') && idxs_a.first_wrong != stk_a->top \
...
static void	ft_get_idxs(t_stack *stk, t_idxs *idx_stk, char stk_name, int fwp)
{
	...
	while (fwp ==  stk->top && stk->top > 1 && i > 0)
	{
		if (!find_flag)
		{
	...
	if (!find_flag && fwp ==  stk->top)
		idx_stk->first_wrong = stk->top;
}
