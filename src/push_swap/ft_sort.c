...
/*   Updated: 2021/11/16 15:23:27 by sde-alva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_push_swap.h"

static void	sort_rev(t_stack *stk_a, t_stack *stk_b, int i)
{
	ft_rotate(stk_a, 'a');
	ft_rotate(stk_a, 'a');
	ft_rotate(stk_a, 'a');
...
static void	sort_a(t_stack *stk_a, int midl_a)
{
...
void	ft_sort_steck_a(t_stack *stk_a, t_stack *stk_b, int len)
{
	int	pivot_a;
	int	pivot_b;
	int	cnt;
	...
	cnt = len / 2;
	pivot_a = quick_pivot(stk_a, len, cnt);
	pivot_b = quick_pivot(stk_a, len, 1);
	while (cnt)
		if (stk_a->values[0] < pivot_a)
		{
			...
			ft_rotate(stk_a, 'a');
	pivot_a = quick_pivot(stk_a, len, 1);
	ft_sort_a(stk_a, pivot_a);
	cnt = stk_b->top;
	if (cnt == 1)
		ft_sort_two_steck_b(stk_a, stk_b);
	else
		ft_sort_three_steck_b(stk_a, stk_b, cnt);
}
...
void	ft_sort_three(t_stack *stk_a, int len)
{
	...
	midl = ft_quick_pivot(stk_a, len, 1);
